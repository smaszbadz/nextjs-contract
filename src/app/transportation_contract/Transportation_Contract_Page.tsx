...
import { Eye, Plus, Copy, RefreshCw, FileText, ChevronDown, ChevronUp, EllipsisVertical, CircleX, NotebookText } from 'lucide-react';
...
// Change DataType to Transportation to use actual data
type DataType = Transportation;
...
const getColumns = (userRole: string | null, permission: string | null): TableColumnsType<DataType> => [
        ...
        render: (_, record) => {
            const items: MenuProps['items'] = [
                ...((userRole === 'ADMIN' || permission === 'Full Access') ? [{
                    key: '1',
                    label: 'Copy',
                    icon: <Copy size={16} className="text-sea-500 hover:text-sea-600" />,
                    onClick: () => handleActionClick('copy', record),
                }] : []),
                ...((!record.isRenewed && (userRole == 'ADMIN' || permission === 'Full Access')) ? [{
                    key: '2',
                    label: 'Renewal',
                    icon: <RefreshCw size={16} className="text-sea-500 hover:text-sea-600" />,
    ...
    },
    {
        ...
        render: (_, record) => (
            <div className="flex justify-center">
                <a href={`/transportation_contract/view/${record.id}`} className="text-sea-500 hover:text-sea-600" title="View">
                    <Eye size={18} />
                </a>
            </div>
        ),
    ...
    },
    {
        ...
        render: (_, record) => (
            record.fileName ? (
                <div className="flex justify-center">
                    <span className="text-lightPurple-500 hover:text-lightPurple-600 cursor-pointer" title="File"
                        onClick={() => handleDownloadFile('Transportation Contract', record.id, record.fileName)}>
                        <FileText size={18} />
                    </span>
                </div>
    ...
    {
        title: 'Logistics Provider (EN)',
        dataIndex: 'logisticsEN',
        key: 'logisticsEN',
        rowSpan: 2,
        className: 'thead-align-center text-nowrap align-top',
        sorter: (a, b) => a.logisticsEN.length - b.logisticsEN.length,
    },
    {
        title: 'Logistics Provider (TH)',
        dataIndex: 'logisticsTH',
        key: 'logisticsTH',
        rowSpan: 2,
        className: 'thead-align-center text-nowrap align-top',
        sorter: (a, b) => a.logisticsTH.length - b.logisticsTH.length,
    },
...
const handleActionClick = (action: string, record: Transportation) => {
    switch (action) {
        case 'copy':
            // Implement copy logic here
            window.location.href = `/transportation_contract/action/${record.id}/copy`;
            break;
        case 'renew':
            // Implement renew logic here  
            window.location.href = `/transportation_contract/action/${record.id}/renew`;
            break;
    }
};


function Transportation_Contract_Page({ userRole, permission, transportation, core_Renewal_Masters,
    core_Notify_Masters, core_Status_Masters, core_PIC_Masters }: Props) {
    ...
    useEffect(() => {
        ...
        const savedColumns = localStorage.getItem('transportationContractColumns');
        if (savedColumns) {
            try {
            ...
            const restoreFormState = () => {
                setFilters(prev => ({ ...prev, logisticsEN: localStorage.getItem('logisticsEN') || '' }));
                setFilters(prev => ({ ...prev, logisticsTH: localStorage.getItem('logisticsTH') || '' }));
    ...
    useEffect(() => {
        ...
        if (typeof window !== 'undefined') {
            ...
            const savedPage = localStorage.getItem('transportationContractCurrentPage');
            const savedPageSize = localStorage.getItem('transportationContractPageSize');
    ...
    useEffect(() => {
        ...
        const stateToLocalStorage = {
            'transportationContractColumns': JSON.stringify(checkedList),
            'logisticsEN': filters.logisticsEN,
            'logisticsTH': filters.logisticsTH,
    ...
    const filteredDataSource = useMemo(() => {
        return transportation?.filter(transportation => {

            const matchLogisticsEN = !filters.logisticsEN ||
                transportation.logisticsEN.toLowerCase().includes(filters.logisticsEN.toLowerCase());

            // Status filter
            const matchStatus = !filters.status || transportation.status === filters.status;

            // Contract Name filter (case-insensitive, partial match)
            const matchContractName = !filters.contractName ||
                transportation.contractName.toLowerCase().includes(filters.contractName.toLowerCase());

            // Start and End Date filter
            const matchDateRange = (!filters.startDate || !filters.endDate) ||
                (transportation.startDate && transportation.endDate &&
                    // Transportation starts before or on the end date
                    new Date(transportation.startDate) <= filters.endDate &&
                    // Transportation ends after or on the start date
                    new Date(transportation.endDate) >= filters.startDate
                );

            const matchLogisticsTH = !filters.logisticsTH ||
                transportation.logisticsTH.toLowerCase().includes(filters.logisticsTH.toLowerCase());

            // Attachment filter
            const matchAttachmentType = !filters.attachmentType ||
                transportation.attachmentType === filters.attachmentType;

            // PIC filter
            const matchPIC = !filters.pic || transportation.pic_ID1 === filters.pic || transportation.pic_ID2 === filters.pic;

            const matchRenewal = !filters.renewal || transportation.renewal === filters.renewal;

            const matchNotify = !filters.notify || transportation.notify === filters.notify;

            const matchContractNo = !filters.contractNo ||
                transportation.contractNo.toLowerCase().includes(filters.contractNo.toLowerCase());

            // Expiry filter
            const matchExpiry = !filters.expiry || (
                (filters.expiry === 'Expires in 7 Days' &&
                    transportation.endDate &&
                    dayjs(transportation.endDate).diff(dayjs(), 'day') <= 7 &&
                    dayjs(transportation.endDate).diff(dayjs(), 'day') > 0
                ) ||
                (filters.expiry === 'Expired' &&
                    transportation.endDate &&
                    dayjs(transportation.endDate).isBefore(dayjs())
                )
            );

            // Combine all filters
            return matchLogisticsEN &&
                matchStatus &&
                matchContractName &&
                matchDateRange &&
                matchLogisticsTH &&
    ...
    const handleTableChange = (newPagination: TablePaginationConfig) => {
        ...
        try {
            localStorage.setItem('transportationContractCurrentPage', newCurrentPage.toString());
            localStorage.setItem('transportationContractPageSize', newPageSize.toString());
        } catch (error) {
            console.error('Error saving pagination to localStorage', error);
        }
    };

    const exportToExcel = async (data: Transportation[], fileName: string = 'transportation_contracts.xlsx') => {
        // Create a new workbook and worksheet
        const workbook = new ExcelJS.Workbook();
        const worksheet = workbook.addWorksheet('Transportation Contracts');
        ...
        data.forEach(record => {
            ...
            visibleColumns.forEach(col => {
                if (col.key === 'contractPeriod' && checkedList.includes('contractPeriod')) {
                    ...
                    const value = record[col.dataIndex as keyof Transportation];
                    if (value instanceof Date) {
                        ...
                        // จัดการกับค่า Boolean
                        rowValues.push(value ? 'Yes' : 'No'); // หรือจะใส่เป็น true/false หรือ 1/0 ก็ได้
                    }
                    else {
    ...
    const exportToPdf = async (data: Transportation[], fileName = 'transportation_contracts.pdf') => {
        ...
        pdf.setFontSize(16);
        pdf.text('Transportation Contract', 14, 15); // Thai title
    ...
    const handleClearAllColumns = () => {
        ...
        try {
            localStorage.setItem('transportationContractColumns', JSON.stringify([]));
        } catch (error) {
        ...
        <main>
            <div className="mx-auto px-5 pb-5">
                <div className="flex items-center gap-2">
                    <span className="text-3xl text-sea-500 font-montserrat font-bold">Transportation Contract Management</span>

                    <div className="space-x-2 ml-auto">
                        {
                            (userRole == "ADMIN" || permission == "Full Access") &&
                            <Link href="/transportation_contract/create" className="btn btn-sea inline-flex items-center font-montserrat">
                                <Plus className="w-5 h-5 mr-2" />
                                Create
                            </Link>
                        }
                        <a
                            href="/Transportation_Manual.pdf"
                            className="btn btn-pinky inline-flex items-center font-montserrat"
                            target="_blank"
                ...
                <div className="card my-8 w-full">
                    <div className="flex justify-between items-center mb-4">
                        <label className="font-montserrat text-sm font-bold text-sea-500 me-2">Filter By</label>
                        <button onClick={() => setIsOpen(!isOpen)} className="text-sea-500 cursor-pointer">
                        ...
                        <div className="flex flex-col gap-4">
                            <div className="grid grid-cols-2 xl:grid-cols-12 items-center gap-4">
                                <div className="col-span-2 xl:col-span-1 2xl:col-span-1 text-left xl:text-right">
                                    <label className="text-title">Logistics Provider <span className="font-normal text-xs">(EN/TH)</span></label>
                                </div>
                                <div className="col-span-2 xl:col-span-4 2xl:col-span-4">
                                    <input value={filters.logisticsEN} onChange={(e) => setFilters(prev => ({ ...prev, logisticsEN: e.target.value }))} className="input-formcontrol" />
                                </div>
                                <div className="col-span-2 xl:col-span-1 2xl:col-span-1 text-left xl:text-right">
                            ...
                            <div className="grid grid-cols-2 xl:grid-cols-12 items-center gap-4">
                                ...
                                <div className="col-span-2 xl:col-span-6 2xl:col-span-6">
                                    <select
                                        className="input-formcontrol"
                            ...
                            <div className="grid grid-cols-2 xl:grid-cols-12 items-center gap-4">
                                ...
                                <div className="col-span-2 xl:col-span-2 2xl:col-span-1">
                                    <button onClick={clearAllFilters} className="btn btn-sm btn-lightPurple items-center">Clear Filter</button>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div className="grid grid-cols-[1fr_auto] gap-2 mt-14 items-stretch">
                    <div >
                        <div className="card h-full">
                            <div className="flex items-start gap-2">
                                <label className="font-montserrat text-sm font-bold text-sea-500 me-2">Columns Displayed</label>
                            ...
                            <div className="w-full mt-2">
                                <div className="grid grid-cols-3 2xl:grid-cols-7 gap-x-4 gap-y-2 w-full mt-2">
                    ...
                    <div >
                        <div className="card h-full">
                            <label className="font-montserrat text-sm font-bold text-sea-500 me-2">Report</label>
                            <div className=" flex item-center flex-nowrap gap-2 mt-2">
                                ...
                                {/* <a href="/transportation_contract/create" className=" btn-outlineMint inline-flex items-center font-montserrat">
...
export default Transportation_Contract_Page;
