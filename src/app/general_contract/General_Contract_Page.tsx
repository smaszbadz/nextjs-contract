...
// Change DataType to General to use actual data
...
const getColumns = (userRole: string | null, permission: string | null): TableColumnsType<DataType> => [
        ...
        render: (_, record) => {
            const items: MenuProps['items'] = [
                ...((userRole === 'ADMIN' || permission === 'Full Access') ? [{
                    key: '1',
                    label: 'Copy',
                    icon: <Copy size={16} className="text-biscuit-500 hover:text-biscuit-600" />,
                    onClick: () => handleActionClick('copy', record),
                }] : []),
                ...((!record.isRenewed && (userRole == 'ADMIN' || permission === 'Full Access')) ? [{
                    key: '2',
                    label: 'Renewal',
                    icon: <RefreshCw size={16} className="text-biscuit-500 hover:text-biscuit-600" />,
                ...
                ...(((record.status === 'Contract active / สัญญาถูกใช้งาน' || record.status === 'Contract is automatically activated / สัญญาถูกใช้งานอัตโนมัติ') && (userRole == 'ADMIN' || permission === 'Full Access')) ? [
                    {
                        key: '3',
                        label: 'Amendment',
                        icon: <ScrollText size={16} className="text-biscuit-500 hover:text-biscuit-600" />,
    ...
    },
    {
        ...
        render: (_, record) => (
            <div className="flex justify-center">
                <a href={`/general_contract/view/${record.id}`} className="text-biscuit-500 hover:text-biscuit-600" title="View">
                    <Eye size={18} />
                </a>
            </div>
        ),
    ...
    },
    {
        ...
        render: (_, record) => (
            record.fileName ? (
                <div className="flex justify-center">
                    <span className="text-mint-500 hover:text-mint-600 cursor-pointer" title="File"
                        onClick={() => handleDownloadFile('General Contract', record.id, record.fileName)}>
                        <FileText size={18} />
                    </span>
                </div>
...

    {
        ...
        render: (_, record) => {
            ...
            if (record.attachmentType === 'New Contract') {
                ...
                return <span className="badge-mint text-xs font-montserrat font-semibold">Amendment</span>
            } else {
                return null;
            }
        }
...
const handleActionClick = (action: string, record: General) => {
    switch (action) {
        case 'copy':
            // Implement copy logic here
            window.location.href = `/general_contract/action/${record.id}/copy`;
            break;
        case 'renew':
            // Implement renew logic here  
            window.location.href = `/general_contract/action/${record.id}/renew`;
            break;
        case 'amendment':
            // Implement amendment logic here
            window.location.href = `/general_contract/amendment/${record.id}`;
            break;
    }
};


function General_Contract_Page({ userRole, permission, generals, core_Renewal_Masters,
    core_Notify_Masters, core_Status_Masters, core_PIC_Masters, general_Usage_Masters, general_Category_Masters }: Props) {
    ...
    useEffect(() => {
        ...
        const savedColumns = localStorage.getItem('generalContractColumns');
        if (savedColumns) {
            try {
            ...
            const restoreFormState = () => {
                ...
                setFilters(prev => ({ ...prev, usage: localStorage.getItem('category') || '' }));
    ...
    useEffect(() => {
        ...
        if (typeof window !== 'undefined') {
            ...
            const savedPage = localStorage.getItem('generalContractCurrentPage');
            const savedPageSize = localStorage.getItem('generalContractPageSize');
    ...
    useEffect(() => {
        ...
        const stateToLocalStorage = {
            'generalContractColumns': JSON.stringify(checkedList),
    ...
    const filteredDataSource = useMemo(() => {
        return generals?.filter(general => {
            ...
            // Contracting  Company EN filter (case-insensitive, partial match)
            const matchCompanyEN = !filters.companyEN ||
                general.companyEN.toLowerCase().includes(filters.companyEN.toLowerCase());

            // Status filter
            const matchStatus = !filters.status || general.status === filters.status;

            // Contract Name filter (case-insensitive, partial match)
            const matchContractName = !filters.contractName ||
                general.contractName.toLowerCase().includes(filters.contractName.toLowerCase());

            // Start and End Date filter
            const matchDateRange = (!filters.startDate || !filters.endDate) ||
                (general.startDate && general.endDate &&
                    // General starts before or on the end date
                    new Date(general.startDate) <= filters.endDate &&
                    // General ends after or on the start date
                    new Date(general.endDate) >= filters.startDate
                );

            // Contracting  Company TH filter
            const matchCompanyTH = !filters.companyTH ||
                general.companyTH.toLowerCase().includes(filters.companyTH.toLowerCase());

            // Attachment filter
            const matchAttachmentType = !filters.attachmentType ||
                general.attachmentType === filters.attachmentType;

            // PIC filter
            const matchPIC = !filters.pic || general.pic_ID1 === filters.pic || general.pic_ID2 === filters.pic;


            // General Type filter
            const matchUsage = !filters.usage || general.usage === filters.usage;


            const matchCategory = !filters.category || general.category === filters.category;



            const matchRenewal = !filters.renewal || general.renewal === filters.renewal;

            const matchContractNo = !filters.contractNo ||
                general.contractNo.toLowerCase().includes(filters.contractNo.toLowerCase());


            const matchNotify = !filters.notify || general.notify === filters.notify;

            // Expiry filter
            const matchExpiry = !filters.expiry || (
                (filters.expiry === 'Expires in 7 Days' &&
                    general.endDate &&
                    dayjs(general.endDate).diff(dayjs(), 'day') <= 7 &&
                    dayjs(general.endDate).diff(dayjs(), 'day') > 0
                ) ||
                (filters.expiry === 'Expired' &&
                    general.endDate &&
                    dayjs(general.endDate).isBefore(dayjs())
                )
            );
    ...
    const handleTableChange = (newPagination: TablePaginationConfig) => {
        ...
        try {
            localStorage.setItem('generalContractCurrentPage', newCurrentPage.toString());
            localStorage.setItem('generalContractPageSize', newPageSize.toString());
        } catch (error) {
            console.error('Error saving pagination to localStorage', error);
        }
    };

    const exportToExcel = async (data: General[], fileName: string = 'general_contracts.xlsx') => {
        // Create a new workbook and worksheet
        const workbook = new ExcelJS.Workbook();
        const worksheet = workbook.addWorksheet('General Contracts');
        ...
        data.forEach(record => {
            ...
            visibleColumns.forEach(col => {
                if (col.key === 'contractPeriod' && checkedList.includes('contractPeriod')) {
                    ...
                    const value = record[col.dataIndex as keyof General];
    ...
    const exportToPdf = async (data: General[], fileName = 'general_contracts.pdf') => {
        ...
        pdf.setFontSize(16);
        pdf.text('General Contract', 14, 15); // Thai title
    ...
    const handleClearAllColumns = () => {
        ...
        try {
            localStorage.setItem('generalContractColumns', JSON.stringify([]));
        } catch (error) {
        ...
        <main>
            <div className="mx-auto px-5 pb-5">
                <div className="flex items-center gap-2">
                    <span className="text-3xl text-biscuit-500 font-montserrat font-bold">General Contract Management</span>

                    <div className="space-x-2 ml-auto">
                        {
                            (userRole == "ADMIN" || permission == "Full Access") &&
                            <Link href="/general_contract/create" className="btn btn-biscuit inline-flex items-center font-montserrat">
                ...
                <div className="card my-8 w-full">
                    <div className="flex justify-between items-center mb-4">
                        <label className="font-montserrat text-sm font-bold text-biscuit-500 me-2">Filter By</label>
                        <button onClick={() => setIsOpen(!isOpen)} className="text-biscuit-500 cursor-pointer">
                        ...
                        <div className="flex flex-col gap-4">
                            <div className="grid grid-cols-2 xl:grid-cols-12 items-center gap-4">
                                <div className="col-span-2 xl:col-span-1 2xl:col-span-1 text-left xl:text-right">
                                    <label className="text-title">Contracting  Company <span className="font-normal text-xs">(EN/TH)</span></label>
                                </div>
                            ...
                            <div className="grid grid-cols-2 xl:grid-cols-12 items-center gap-4">
                                ...
                                <div className="col-span-2 xl:col-span-2 2xl:col-span-1">
                                    <button onClick={clearAllFilters} className="btn btn-sm btn-mint items-center">Clear Filter</button>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div className="grid grid-cols-[1fr_auto] gap-2 mt-14 items-stretch">
                    <div >
                        <div className="card h-full">
                            ...
                            <div className="w-full mt-2">
                                <div className="grid grid-cols-4 2xl:grid-cols-6 gap-x-4 gap-y-2 w-full mt-2">
                    ...
                    <div>
                        <div className="card h-full">
                            <label className="font-montserrat text-sm font-bold text-biscuit-500 me-2">Report</label>
                            <div className=" flex item-center flex-nowrap gap-2 mt-2">
                                ...
                                {/* <a href="/general_contract/create" className=" btn-outlineMint inline-flex items-center font-montserrat">
...
export default General_Contract_Page;
