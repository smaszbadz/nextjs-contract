...
// Change DataType to Insurance to use actual data
type DataType = Insurance;
...
const getColumns = (userRole: string | null, permission: string | null): TableColumnsType<DataType> => [
        ...
        render: (_, record) => {
            const items: MenuProps['items'] = [
                ...((userRole === 'ADMIN' || permission === 'Full Access') ? [{
                    key: '1',
                    label: 'Copy',
                    icon: <Copy size={16} className="text-lightPurple-500 hover:text-lightPurple-600" />,
                    onClick: () => handleActionClick('copy', record),
                }] : []),
                ...((!record.isRenewed && (userRole == 'ADMIN' || permission == "Full Access")) ? [{
                    key: '2',
                    label: 'Renewal',
                    icon: <RefreshCw size={16} className="text-lightPurple-500 hover:text-lightPurple-600" />,
                ...
                ...(((record.status === 'Contract active / สัญญาถูกใช้งาน' || record.status === 'Contract is automatically activated / สัญญาถูกใช้งานอัตโนมัติ') && (userRole == 'ADMIN' || permission == "Full Access")) ? [
                    {
                        key: '3',
                        label: 'Amendment',
                        icon: <ScrollText size={16} className="text-lightPurple-500 hover:text-lightPurple-600" />,
    ...
    },
    {
        ...
        render: (_, record) => (
            <div className="flex justify-center">
                <a href={`/insurance_contract/view/${record.id}`} title="View">
                    <Eye size={18} className="text-lightPurple-500 hover:text-lightPurple-600 cursor-pointer" />
                </a>
            </div>
        ),
        rowSpan: 2
    },
    {
        ...
        render: (_, record) => (
            record.fileName ? (
                <div className="flex justify-center">
                    <span className="text-pinky-500 hover:text-pinky-600 cursor-pointer" title="File"
                        onClick={() => handleDownloadFile('Insurance Contract', record.id, record.fileName)}>
                        <FileText size={18} />
                    </span>
                </div>
    ...
    },
    {
        ...
        rowSpan: 2,
        className: 'thead-align-center text-nowrap align-top',
        sorter: (a, b) => a.companyEN.length - b.companyEN.length,
    },
    {
        title: 'Company (TH)',
        dataIndex: 'companyTH',
        ...
        rowSpan: 2,
        className: 'thead-align-center text-nowrap align-top',
        sorter: (a, b) => a.companyTH.length - b.companyTH.length,
    },
    {
        title: 'Insured Name',
        ...
        rowSpan: 2,
        className: 'thead-align-center text-nowrap align-top',
        sorter: (a, b) => a.insuredName.length - b.insuredName.length,
    },
...

    {
        ...
        rowSpan: 2,
        className: 'thead-align-center text-nowrap  align-top text-center',
        sorter: (a, b) => (a.stampDuty || 0) - (b.stampDuty || 0),
        render: (_, record) => {
            const data = record.stampDuty;
            if (data !== null && data !== undefined && data > 0) {
                return Number(data).toLocaleString('en-US', {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2
...
const handleActionClick = (action: string, record: Insurance) => {
    switch (action) {
        case 'copy':
            // Implement copy logic here
            window.location.href = `/insurance_contract/action/${record.id}/copy`;
            break;
        case 'renew':
            // Implement renew logic here  
            window.location.href = `/insurance_contract/action/${record.id}/renew`;
            break;
        case 'amendment':
            // Implement amendment logic here
            window.location.href = `/insurance_contract/amendment/${record.id}`;
            break;
    }
};
...
function Insurance_Contract_Page({ userRole, permission, insurances,
    core_Status_Masters, core_PIC_Masters, core_Branch_Masters, insurance_Code_Masters }: Props) {
    ...
    useEffect(() => {
        ...
        const savedColumns = localStorage.getItem('insuranceContractColumns');
        if (savedColumns) {
            try {
            ...
            const restoreFormState = () => {
                setFilters(prev => ({ ...prev, companyEN: localStorage.getItem('companyEN') || '' }));
                setFilters(prev => ({ ...prev, companyTH: localStorage.getItem('companyTH') || '' }));
                ...
                setFilters(prev => ({ ...prev, code: localStorage.getItem('code') || '' }));
                setFilters(prev => ({ ...prev, branch: localStorage.getItem('branch') || '' }));
                setFilters(prev => ({ ...prev, insuredName: localStorage.getItem('insuredName') || '' }));
                setFilters(prev => ({ ...prev, premise: localStorage.getItem('premise') || '' }));
    ...
    useEffect(() => {
        ...
        if (typeof window !== 'undefined') {
            ...
            const savedPage = localStorage.getItem('insuranceContractCurrentPage');
            const savedPageSize = localStorage.getItem('insuranceContractPageSize');
    ...
    useEffect(() => {
        ...
        const stateToLocalStorage = {
            'insuranceContractColumns': JSON.stringify(checkedList),
            'companyEN': filters.companyEN,
            'companyTH': filters.companyTH,
    ...
    const filteredDataSource = useMemo(() => {
        return insurances?.filter(insurance => {
            ...
            // Company EN filter (case-insensitive, partial match)
            const matchCompanyEN = !filters.companyEN ||
                insurance.companyEN.toLowerCase().includes(filters.companyEN.toLowerCase());

            // Status filter
            const matchStatus = !filters.status || insurance.status === filters.status;

            // Contract Name filter (case-insensitive, partial match)
            const matchContractName = !filters.contractName ||
                insurance.contractName.toLowerCase().includes(filters.contractName.toLowerCase());

            // Start and End Date filter
            const matchDateRange = (!filters.startDate || !filters.endDate) ||
                (insurance.startDate && insurance.endDate &&
                    // Insurance starts before or on the end date
                    new Date(insurance.startDate) <= filters.endDate &&
                    // Insurance ends after or on the start date
                    new Date(insurance.endDate) >= filters.startDate
                );

            // Company TH filter
            const matchCompanyTH = !filters.companyTH ||
                insurance.companyTH.toLowerCase().includes(filters.companyTH.toLowerCase());

            // Attachment filter
            const matchAttachmentType = !filters.attachmentType ||
                insurance.attachmentType === filters.attachmentType;

            // PIC filter
            const matchPIC = !filters.pic || insurance.pic_ID1 === filters.pic || insurance.pic_ID2 === filters.pic;


            // Insurance Type filter
            const matchCode = !filters.code || insurance.code === filters.code;
            const matchBranch = !filters.branch || insurance.branch === filters.branch;

            const matchInsuredName = !filters.insuredName ||
                insurance.insuredName.toLowerCase().includes(filters.insuredName.toLowerCase());

            const matchPremise = !filters.premise ||
                insurance.premise.toLowerCase().includes(filters.premise.toLowerCase());

            const matchContractNo = !filters.contractNo ||
                insurance.contractNo.toLowerCase().includes(filters.contractNo.toLowerCase());
            ...
            const matchExpiry = !filters.expiry || (
                (filters.expiry === 'Expires in 7 Days' &&
                    insurance.endDate &&
                    dayjs(insurance.endDate).diff(dayjs(), 'day') <= 7 &&
                    dayjs(insurance.endDate).diff(dayjs(), 'day') > 0
                ) ||
                (filters.expiry === 'Expired' &&
                    insurance.endDate &&
                    dayjs(insurance.endDate).isBefore(dayjs())
                )
            );

            // Combine all filters
            return matchCompanyEN &&
                matchStatus &&
                matchContractName &&
                matchDateRange &&
                matchCompanyTH &&
    ...
    const handleTableChange = (newPagination: TablePaginationConfig) => {
        ...
        try {
            localStorage.setItem('insuranceContractCurrentPage', newCurrentPage.toString());
            localStorage.setItem('insuranceContractPageSize', newPageSize.toString());
        } catch (error) {
            console.error('Error saving pagination to localStorage', error);
        }
    };

    const exportToExcel = async (data: Insurance[], fileName: string = 'insurance_contracts.xlsx') => {
        // Create a new workbook and worksheet
        const workbook = new ExcelJS.Workbook();
        const worksheet = workbook.addWorksheet('Insurance Contracts');
        ...
        data.forEach(record => {
            ...
            visibleColumns.forEach(col => {
                if (col.key === 'contractPeriod' && checkedList.includes('contractPeriod')) {
                    ...
                    const value = record[col.dataIndex as keyof Insurance];
    ...
    const exportToPdf = async (data: Insurance[], fileName = 'insurance_contracts.pdf') => {
        ...
        pdf.setFontSize(16);
        pdf.text('Insurance Contract', 14, 15); // Thai title
    ...
    const handleClearAllColumns = () => {
        ...
        try {
            localStorage.setItem('insuranceContractColumns', JSON.stringify([]));
        } catch (error) {
        ...
        <main>
            <div className="mx-auto px-5 pb-5">
                <div className="flex items-center gap-2">
                    <span className="text-3xl text-lightPurple-500 font-montserrat font-bold">Insurance Contract Management</span>

                    <div className="space-x-2 ml-auto">
                        {
                            (userRole == "ADMIN" || permission == "Full Access") &&
                            <Link href="/insurance_contract/create" className="btn btn-lightPurple inline-flex items-center font-montserrat">
                ...
                <div className="card my-8 w-full">
                    <div className="flex justify-between items-center mb-4">
                        <label className="font-montserrat text-sm font-bold text-lightPurple-500 me-2">Filter By</label>
                        <button onClick={() => setIsOpen(!isOpen)} className="text-lightPurple-500 cursor-pointer">
                        ...
                        <div className="flex flex-col gap-4">
                            <div className="grid grid-cols-2 xl:grid-cols-12 items-center gap-4">
                                <div className="col-span-2 xl:col-span-1 2xl:col-span-1 text-left xl:text-right">
                                    <label className="text-title">Company <span className="font-normal text-xs">(EN/TH)</span></label>
                                </div>
                                <div className="col-span-2 xl:col-span-4 2xl:col-span-4">
                                    <input value={filters.companyEN} onChange={(e) => setFilters(prev => ({ ...prev, companyEN: e.target.value }))} className="input-formcontrol" />
                                </div>
                                <div className="col-span-2 xl:col-span-1 2xl:col-span-1 text-left xl:text-right">
                            ...
                            <div className="grid grid-cols-2 xl:grid-cols-12 items-center gap-4">
                                <div className="col-span-2 xl:col-span-1 2xl:col-span-1 text-left xl:text-right">
                                    <label className="text-title">Code</label>
                                </div>
                                <div className="col-span-2 xl:col-span-4">
                                    <div className="flex items-center">
                                        <div className="flex-initial w-1/4 2xl:w-2/6">
                                        ...
                                        <div className="text-left xl:text-right pl-6 pr-3">
                            ...
                            <div className="grid grid-cols-2 xl:grid-cols-12 items-center gap-4">
                                ...
                                <div className="col-span-2 xl:col-span-1 2xl:col-span-1 text-left xl:text-right">
                                    <label className="text-title">The Insured</label>
                                </div>
                                <div className="col-span-2 xl:col-span-6 2xl:col-span-5">
                                    <input value={filters.insuredName} onChange={(e) => setFilters(prev => ({ ...prev, insuredName: e.target.value }))} className="input-formcontrol" />
                                </div>
                            ...
                            <div className="grid grid-cols-2 xl:grid-cols-12 items-center gap-4">
                                ...
                                <div className="col-span-2 xl:col-span-9 2xl:col-span-7">
                                    <input value={filters.premise} onChange={(e) => setFilters(prev => ({ ...prev, premise: e.target.value }))} className="input-formcontrol" />
                                </div>
                                <div className="col-span-2 xl:col-span-2 2xl:col-span-1">
                                    <button onClick={clearAllFilters} className="btn btn-sm btn-pinky items-center">Clear Filter</button>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div className="grid grid-cols-[1fr_auto] gap-2 mt-14 items-stretch">
                    <div >
                        <div className="card h-full">
                            <div className="flex items-start gap-2">
                                <label className="font-montserrat text-sm font-bold text-lightPurple-500 me-2">Columns Displayed</label>
                    ...
                    <div >
                        <div className="card h-full">
                            <label className="font-montserrat text-sm font-bold text-lightPurple-500 me-2">Report</label>
                            <div className=" flex item-center flex-nowrap gap-2 mt-2">
                                ...
                                {/* <a href="/insurance_contract/create" className=" btn-outlineMint inline-flex items-center font-montserrat">
